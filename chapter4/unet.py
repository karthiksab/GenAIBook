...
def get_timestep_embedding(timesteps, embedding_dim: int): # Build Sinusoidal Embeddings
    """
...
class ResNetBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, dropout_rate=0.1):
        ...
        self.conv1 = nn.Conv2d(in_ch, out_ch, 3, stride=1, padding=1) # first convolution
        self.dense = nn.Linear(512, out_ch) # dense layer 512-temb
        self.conv2 = nn.Conv2d(out_ch, out_ch, 3, stride=1, padding=1) # second convolutoin
        
        if not (in_ch == out_ch):
            self.nin = Nin(in_ch, out_ch) # When output channel is different to input channel
            
        self.dropout_rate = dropout_rate # drop out 
        self.nonlinearity = torch.nn.SiLU() # activiation function
    ...
    def forward(self, x, temb):
        ...
        h = self.nonlinearity(nn.functional.group_norm(x, num_groups=32)) # group normalization
        h = self.conv1(x) #first convolution
        ...
        if not (x.shape[1] == h.shape[1]):
            x = self.nin(x) # When output channel is different to input channel
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        h = nn.functional.group_norm(x, num_groups=32) # group normalization
        q = self.Q(h)
        k = self.K(h)
        v = self.V(h)
